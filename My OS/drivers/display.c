...
void print_string_at(char *string, int col, int row)
{
...
void print_int(int num)
{
    ...
    while (weight >= 1)
    {
        int temp = (num / weight) - ((num / (weight * 10)) * 10);
        for (i = 0; i < 10; i++)
        {
            if (temp == val_int[i])
            {
...
void print_int_at(int num, int col, int row)
{
    char *val_char[10] = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0"};
    int val_int[10] = {1, 2, 3, 4, 5, 6, 7, 8, 9, 0};
    int weight = 1;
